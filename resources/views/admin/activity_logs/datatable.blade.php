<script type="text/javascript">
    $(function() {
        let dataUrl, isActivitylogs = false;

        @if (request()->is('activity-logs*'))
            dataUrl = "{{ route('activity_logs.index') }}";
            isActivitylog = true;
        @else
            dataUrl = "{{ route('activity_logs.your_activity_log') }}";
            isActivitylog = false;
        @endif
        ...
        var table = new $('#activity-logs-table').DataTable({
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        return `<div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-end">
                                    <a href="javascript:void(0)" data-id="${data}" data-action="{{ url('/activity-logs') }}/${data}" class="dropdown-item deleteRecord ">
                                        Delete
                                    </a>
                                </div>
                            </div>`;
            ...
            drawCallback: function(settings) {
                ...
                //check if activity log page
                if (isActivitylog) {
                    allActivityLogTotal = apiData?.recordsTotal ?? 0;
                    yourActivityLogTotal = apiData?.yourActivityLogTotal ?? 0;
                } else {
                    allActivityLogTotal = apiData?.activityLogTotal ?? 0;
                    yourActivityLogTotal = apiData?.recordsTotal ?? 0;
                }
                activityLogOnlyTrashedTotal = apiData?.activityLogOnlyTrashedTotal ?? 0;
